...
export default function Review({ product }) {
  ...
  useEffect(() => {
    ...
    product &&
      product._id &&
      fetch(`https://polo-chino-back.vercel.app/polos/getOne/${product._id}`)
  ...
  const handleClick = () => {
    fetch(`https://polo-chino-back.vercel.app/reviews/post/${user.token}`, {
      method: "POST",
    ...
    <div className="flex gap-10 mx-8 my-12 overflow-hidden">
      <blockquote className="rounded-lg bg-gray-50 p-6 shadow-sm w-[400px] sm:p-8">
        <div className="flex flex-col gap-4">
          ...
          <div>
            <div className="flex  gap-0.5 ">{personalStars}</div>
          </div>
        </div>
        <div>
          <input
